...
getPodsResult=$(kubectl get pods)
if [ $? -ne 0 ]; then
echo "Couldn't get " 
exit 1
fi
...
url="$(kubectl describe pod "$pod" | grep PublicServerUrl | head -1 | awk '{print $2}')"

echo "Setting image $1 on the $pod"
kubectl set image statefulset/"$statefulsetname" ravendb-container=ravendb/ravendb:"$1"
...
status="$(kubectl get pods | grep "$pod" | awk '{print $3}')"
sleep 3
done
